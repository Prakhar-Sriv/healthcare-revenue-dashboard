...
# Revenue Leakage → difference between expected hospital revenue and actual revenue received
df["Revenue_Leakage"] = df["Expected_Revenue"] - df["Actual_Revenue"]

# Revenue Leakage Index (%) → how much revenue was lost relative to expected revenue
df["Revenue_Leakage_Index"] = (
    df["Revenue_Leakage"] /
    df["Expected_Revenue"].replace(0, np.nan)
) * 100

# Charge Capture Efficiency → measures how effectively services were converted into billable charges
df["Charge_Capture_Efficiency"] = (
    df["Billing_Amount"] /
    df["Expected_Revenue"].replace(0, np.nan)
) * 100

# Accounts Receivable Days → number of days taken to settle claims after submission
...
# Revenue at Risk → billed revenue that has not yet been collected
